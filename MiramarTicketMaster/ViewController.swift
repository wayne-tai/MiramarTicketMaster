...
class ViewController: UIViewController {
    ...
    fileprivate var targetMovieName: String { return "Avengers" }
    
    fileprivate var targetMovieDate: String { return "2018-04-28" }
    
//    fileprivate var targetMovieDate: String { return "2018-04-29" }
    
    fileprivate var targetMovieHour: Int { return 18 }
    
    fileprivate var targetMovieMin: Int { return 30 }
    ...
    fileprivate var cinemaId: String? = UserDefaults.standard.string(forKey: Environments.UserDefaultsKey.CinemaId) {
        didSet {
            UserDefaults.standard.set(cinemaId, forKey: Environments.UserDefaultsKey.CinemaId)
            UserDefaults.standard.synchronize()
        }
    }
    
    fileprivate var sessionId: String? = UserDefaults.standard.string(forKey: Environments.UserDefaultsKey.SessionId) {
        didSet {
            UserDefaults.standard.set(cinemaId, forKey: Environments.UserDefaultsKey.SessionId)
            UserDefaults.standard.synchronize()
        }
    }
        ...
        didSet {
            ...
            case .showTimeReceived(let cinemas):
                ...
                DispatchQueue.global().async {
                    ...
                    guard let movie = ciname.movieList.first(where: { $0.movieEName.contains(self.targetMovieName) }) else {
...
//            case .orderConfirmReceived(let orderConfirm):
    ...
    fileprivate func getAnnounce() {
        self.screenLogger.log("Requesting for announce...\n")
        ...
        API.getAnnounce()
            ...
            .subscribe(
                onNext: { (key) in
                    self.stopAnyTimer()
                    self.key = key.data
                    self.currentState = .announceReceived(key: key.data)
            },
                onError: { (error) in
                    self.screenLogger.log("Request for announce failed...\n\n")
...
//    fileprivate func getTargetTicketType(by ticketTypes: [TicketType]) {
//    }
